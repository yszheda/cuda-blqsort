...
// Kernels use operator< directly (no custom comparator)
#include "blqs.hpp"
...
void do_sort(T* d_data, int n) {
    ...
    if (n <= BLOCK_SIZE) {
        ...
        quicksort_shared<T, 1024><<<1, threads>>>(d_data, n);
    ...
    partition_kernel<T><<<num_blocks, BLOCK_SIZE>>>(
        d_data, n, d_bucket_counts, nullptr, num_buckets);
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    for (int b = 0; b < num_blocks; b++) {
        ...
        if (lower_count > 1) do_sort(d_data + chunk_start, lower_count);
        if (chunk_size - lower_count > 1) do_sort(d_data + chunk_start + lower_count, chunk_size - lower_count);
    }
}
...
void do_sort_by_key(K* d_keys, V* d_values, int n) {
    ...
    CUDA_CHECK(cudaMalloc(&d_bucket_counts, num_blocks * num_buckets * sizeof(int)));
    CUDA_CHECK(cudaMemset(d_bucket_counts, 0, num_blocks * num_buckets * sizeof(int)));
    kv_partition_kernel<K, V><<<num_blocks, BLOCK_SIZE>>>(
        d_keys, d_values, n, d_bucket_counts, nullptr, num_buckets);
    CUDA_CHECK(cudaDeviceSynchronize());
    std::vector<int> h_bucket_counts(num_blocks * num_buckets);
    CUDA_CHECK(cudaMemcpy(h_bucket_counts.data(), d_bucket_counts,
        num_blocks * num_buckets * sizeof(int), cudaMemcpyDeviceToHost));
    ...
    for (int b = 0; b < num_blocks; b++) {
        ...
        int chunk_size = ((n - chunk_start) < BLOCK_SIZE) ? (n - chunk_start) : BLOCK_SIZE;
        int lower_count = h_bucket_counts[b * num_buckets + 0];
        if (lower_count > 1) do_sort_by_key(d_keys + chunk_start, d_values + chunk_start, lower_count);
        if (chunk_size - lower_count > 1) do_sort_by_key(d_keys + chunk_start + lower_count, d_values + chunk_start + lower_count, chunk_size - lower_count);
    }
}

// ── Public API implementations (comparator parameter accepted but ignored) ──
